...
public class Menu {
	...
	private void inicializarVariables() {
		diaTermino = ControlEmpresa.verificarDiaTermino();
	}
	...
	private void llenarTablaOfertasSeleccionadas() {
		ArrayList<Oferta>listaOfertas = obtenerOfertaDiaSeleccionado(fecha);
		if (listaOfertas!=null) {
		for(Oferta of : listaOfertas) {
			Object[]fila = new Object[6];
			fila[0]=of.getOferente();
		...
		botonTerminar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!diaTermino) {
					...
					ControlEmpresa.obtenerSolucion();
					for(Oferta oferta : ControlEmpresa.getOfertaSolucion()) {
						InfoOfertas.guardarOfertasSeleccionadas(oferta);
					}
			...
			public void actionPerformed(ActionEvent e) {
				if(!diaTermino) {
					if(verificarDatos()) {
						...
						String fechaManiana = fecha.format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
						
						Oferta oferta = new Oferta(oferente, horaDesde, horaHasta, monto, tipoShow, fechaManiana);
						
						if(!ControlEmpresa.verificarOfertaRepetida(oferta)) {
							ControlEmpresa.guardar(oferta);
	...
	private ArrayList<Oferta> obtenerOfertaDiaSeleccionado(String fecha){
